...
async function main() {
  ...
  try {
    ...
    const treePanel = await page.$('.component-tree-panel')
    const floorPanel = await page.$('.floor-list-panel')
    console.log('Tree panel visible:', !!treePanel)
    console.log('Floor panel visible:', !!floorPanel)

    await page.screenshot({ path: path.join(__dirname, '..', 'glb-test-screenshot.png') })
    console.log('Screenshot saved to glb-test-screenshot.png')
  } finally {
    await browser.close()
  }
}
