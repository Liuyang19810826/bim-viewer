...
async function main() {
  ...
  try {
    ...
    await clickButton('场景树')
    await clickButton('统计')
    await clickButton('属性')

    const sceneTreePanel = await page.$('.scene-tree-panel')
    const modelStatsPanel = await page.$('.model-stats-panel')
    const nodePropertiesPanel = await page.$('.node-properties-panel')
    console.log('Scene tree panel visible:', !!sceneTreePanel)
    console.log('Model stats panel visible:', !!modelStatsPanel)
    console.log('Node properties panel visible:', !!nodePropertiesPanel)

    const firstTreeRow = await page.$('.scene-tree-panel .tree-row')
    if (firstTreeRow) {
      await firstTreeRow.click()
      await new Promise((r) => setTimeout(r, 500))
      const selected = await page.$('.scene-tree-panel .tree-row.selected')
      console.log('Tree node selected:', !!selected)
    }

    const statsText = await page.evaluate(() => {
      const panel = document.querySelector('.model-stats-panel')
      return panel ? panel.textContent : ''
    })
    console.log('Stats panel includes 顶点数:', statsText.includes('顶点数'))

    await page.screenshot({ path: path.join(__dirname, '..', 'phase1-test-screenshot.png') })
    console.log('Screenshot saved to phase1-test-screenshot.png')
  } finally {
    await browser.close()
  }
}
